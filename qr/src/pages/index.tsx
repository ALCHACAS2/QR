...
import Report from "./api/QR/flotante";
    ...
    return (
        <div className="flex justify-center items-center h-full bg-gray-800">
            <div className="w-4/6 flex flex-col justify-center items-center">
                <button className="print:hidden bg-red-500" onClick={handlePrint}>
                    {" "}
                    IMPRIMIR
                </button>
                {showReport && <Report onClose={handleCloseAbout} />}
                ...
                <div className="w-screen flex justify-center items-center">
                    <QrScanner onDecode={handleScanResult} onError={(error) => console.log(error?.message)} />
                </div>
                {scannedValue && (
